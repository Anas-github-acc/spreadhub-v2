...
def mock_sheet():
    return Sheet(
        ...
        properties=SheetProperties(
            sheet_id=0, title="Sheet1", index=0, sheet_type="GRID"
        ),
        data=SheetData(startRow=0, startColumn=0, rowData={}),
...
class TestSpreadsheetRoutes:
    def test_verify_token_valid(self):
        result = client.get(
            ...
            headers={"Authorization": "Bearer anas_token"},
        )
    ...
    async def test_create_spreadsheet_success(
        self, mock_sheet_service, mock_spreadsheet
    ):
        mock_sheet_service.createSpreadsheet = AsyncMock(return_value=mock_spreadsheet)

        response = client.post(
            "/api/v1/spreadsheets/create", json={"user_id": "anas1234"}
        )
    ...
    async def test_create_sheet_success(self, mock_sheet_service, mock_sheet):
        ...
        response = client.post(
            "/api/v1/spreadsheets/test123/createSheet", json={"user_id": "anas1234"}
        )
    ...
    async def test_get_spreadsheet_success(
        self, mock_sheet_service, mock_spreadsheet, valid_token
    ):
    ...
    async def test_get_spreadsheet_redirect_invalid_gid(
        self, mock_sheet_service, mock_spreadsheet, valid_token
    ):
        mock_sheet_service.getSpreadsheet = AsyncMock(return_value=mock_spreadsheet)
    ...
    async def test_get_spreadsheet_redirect_unauthorized_edit(
        self, mock_sheet_service, mock_spreadsheet, valid_token
    ):
